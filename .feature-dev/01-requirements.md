...
| ОВ-12 | **Сотрудники не входят в систему; публичные формы по email + модуль «Пользователи»** | **ОТЛОЖЕН заказчиком 19.07.2026 — делать позже** | Разворот модели авторизации из ТЗ. Учителя/работники НЕ логинятся: заполняют публичную форму, идентификация по email `@npuu.uz` (валидация домена). Админ/завсклад заводит сотрудников (ФИО, email, роль, категория) в модуле «Пользователи» (М7, пока нет UI/эндпоинтов). Открытые вопросы к реализации (не решены): (1) email на форме должен совпадать с заведённым пользователем (чтобы взять категорию→кассу) — вероятно да; (2) какие формы публичные — заявка на товар и/или расход денег; (3) защита публичных форм (без проверки во внутренней сети / код на email). **Отложено — сейчас другие задачи.** Затрагивает: auth (публичные эндпоинты), новый модуль users CRUD, поле email у `users` (правка контракта — колонки email сейчас нет). |

| ОВ-11 | **Редактирование и удаление уведомлений** | **ЗАКРЫТ заказчиком 19.07.2026** | Полный свод правил ниже (раздел «Правила редактирования и удаления уведомления»). |
| — | Момент перехода `notifications.draft → in_progress` | **ПЕРЕСМОТРЕН 19.07.2026** | Ранее (этап 2) ошибочно: «первое приобретение переводит в работу». **Верно: завсклад вручную "отправляет в работу"** (действие submit), независимо от закупок — иначе состояние «в работе без приобретений» из правила удаления ОВ-11 недостижимо. Совпадает с §4.1 (в работе = остаток > 0, есть с момента отправки). Приобретения оформляются ТОЛЬКО против уведомления «в работе», не против черновика. |

### Правила редактирования и удаления уведомления (ОВ-11, решение заказчика 19.07.2026)

**Статусная модель уточнена:**
- `черновик` — завсклад составляет; приобретений быть не может.
- `в работе` — завсклад **вручную отправил** (submit); остаток к приобретению > 0; приобретений может ещё не быть.
- `закрыт` — всё приобретено (авто, SV-7).

**Редактирование (SV-10):**

| Статус | Что можно | Что нельзя |
|---|---|---|
| Черновик | **всё** — строки, количества, тексты | — |
| В работе | **только аддитивно**: добавлять товары, увеличивать `qty_requested`; править тексты (`comment`, `body_text`, `division_name`) | уменьшать `qty_requested` ниже уже приобретённого; удалять строки, по которым есть приобретения |
| Закрыт | ничего | любые правки |

**Удаление (SV-11) — ключевое правило заказчика:**
> Уведомление можно удалить **тогда и только тогда, когда у него НЕТ ни одного связанного приобретения.**
- Черновик — приобретений нет по определению → **удаляется**.
- В работе **без приобретений** → **удаляется** (ничего не куплено, движений склада нет, документ инертен).
- В работе **с приобретениями** → **НЕЛЬЗЯ** (приобретения породили движения склада — append-only, ADR-1).
- Закрыт → нельзя (по определению всё приобретено).

Это **первое разрешённое удаление документа** в системе (SV-8 запрещает физудаление только справочников/НСИ). Безопасно: уведомление без приобретений не создало ни одного `stock_movement`, удаляется вместе со своими `notification_items` (CASCADE), учёт не задевает. Проверка «нет приобретений» — в сервисе + защита FK `acquisitions.notification_id → notifications` c `ondelete=RESTRICT` (уже стоит: не даст удалить уведомление, на которое ссылается приобретение).
