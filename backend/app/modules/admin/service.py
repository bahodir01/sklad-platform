"""Сервис М7 «Интеграции» (спека15 §5а): проверка и сохранение внешних ключей.
  ...
  ai_search: полноценный вызов Google Gemini на этом этапе НЕ обязателен (по
             прямому указанию спеки) — лёгкая проверка формата ключа (непустая
             строка разумной длины, без привязки к конкретному вендорскому
             префиксу — провайдер менялся, см. .feature-dev/15f-gemini-swap.md).
             Реальный вызов делает modules/ai/service.py при первом
             использовании поиска (спека15 §3a).
...
# Требование спеки §5а: ключ ИИ-поиска — облегчённая проверка формата, без
# реального вызова. Провайдер менялся (Anthropic → Google Gemini, см.
# .feature-dev/15f-gemini-swap.md) — жёсткую привязку к вендорскому префиксу
# ("sk-ant-...") намеренно убрали: ключи Google AI Studio обычно начинаются
# с "AIza", но это не документированный контракт, на который стоит жёстко
# полагаться как на единственно верный. Вместо префикса — мягкая проверка
# длины: достаточно, чтобы отсечь пустой/обрезанный ввод, не будучи излишне
# строгой к конкретному формату ключа.
_AI_SEARCH_KEY_MIN_LEN: Final[int] = 20


def mask_secret(kind: str, plain: str) -> str:
    """`123456:AAE••••1234` (telegram, формат `<bot_id>:<token>`) /
    `AIzaSyF••••ab12` (ai_search и любой другой kind без двоеточия).
...
def _verify_ai_search_key(secret: str) -> str:
    """Облегчённая проверка формата ключа Google Gemini (спека15 §5а) — без
    реального тестового вызова API на этом этапе. Мягкая проверка (длина),
    без привязки к конкретному вендорскому префиксу — см. комментарий у
    ``_AI_SEARCH_KEY_MIN_LEN``."""
    if len(secret) < _AI_SEARCH_KEY_MIN_LEN:
        raise ValidationError(
            "Похоже, это не ключ API — ключ слишком короткий"
        )
