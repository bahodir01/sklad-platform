...
class GetCustomerOrdersHandler(QueryHandler[GetCustomerOrders, PaginatedResult[OrderView]]):
    ...
    async def handle(self, query: GetCustomerOrders) -> PaginatedResult[OrderView]:
        async with self.read_db.acquire() as conn:
            ...
            # SAFETY: `where_clause` is assembled only from hard-coded literals
            # below — never from user input. All user-supplied values are passed
            # as bound parameters ($1, $2, ...). Do NOT interpolate request data
            # into this string, or you reintroduce SQL injection.
